/// Headless audio example: push APU samples from an x-nes emulator
/// through x-audio's buffer.
///
...
fn main() {
    ...
    // Match x-nes's default sample rate
    ...
    while frame_count < max_frames {
        ...
        if n > 0 {
            let pushed = buf.push(&bus.apu.audio_samples[..n]);
    ...
    println!("  Buffer available: {}", buf.available());
